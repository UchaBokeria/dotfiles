...
class ArchPilotWindow(Gtk.ApplicationWindow):
    ...
    def _commit_rename(self, *_args) -> None:
        ...
        self.renaming = None
        self.rename_text = ""
        # Re-read so the row shows the stored name, trimmed the way it was
        # saved. The daemon writes the file, so wait a beat before re-reading
        # or the list shows the previous name back again.
        def reread() -> bool:
            self._all_history = history_ctx.recent(limit=300)
            if self.search_active:          # stay on the history page
                self._refresh_search()
            return False

        GLib.timeout_add(120, reread)
        self._toast("renamed")
