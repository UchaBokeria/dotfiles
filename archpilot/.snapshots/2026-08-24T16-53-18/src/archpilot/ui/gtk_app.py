...
#: A plain multiplication sign, not the nerd-font \uf00d. That glyph has
#: its own bearings and sat low and right of a round hover background,
#: which looked like the highlight was misaligned.
ICON_CLOSE = "\N{MULTIPLICATION SIGN}"
...
COMMANDS = {
    # ctrl-n is the multicursor key, as in vim-visual-multi, so a new chat
    # takes ctrl-t - the "new tab" convention everywhere else.
    "ctrl-t": ("new", {}),
...
class ArchPilotWindow(Gtk.ApplicationWindow):
    ...
    def _render_tabs(self, state: dict) -> None:
        ...
        for index, tab in enumerate(tabs):
            ...
            # Centred and fixed, or the button stretches to the tab's full
            # height and the hover lights up a tall slab beside the label.
    ...
    def _render_prompt(self) -> None:
        ...
        if self.buffer.block_edit and mode == INSERT:
            # A caret on every line being typed into. One caret while three
            # lines change is unsettling - it reads as if the other two are
            # doing it by themselves.
            points, _count, _kind = self.buffer.block_edit
            body = self._carets_markup(text, sorted(points))
        elif self.buffer.multi:
            # Every cursor's selection, highlighted at once - otherwise the
            # only sign that ctrl-n did anything is the caret jumping.
            body = self._block_markup(text, sorted(self.buffer.multi))
        elif mode == VBLOCK:
            # A column rectangle, one highlighted span per line. Painting
            # selection() instead - a single linear range - lit up whole lines
            # between the corners, so the mode said V-B while the screen showed
            # something indistinguishable from ordinary visual.
            body = self._block_markup(text, self.buffer.block_ranges())
        elif mode in (VISUAL, VLINE):
        ...
        self.vim_chip.set_text(
            f"\N{MULTIPLICATION SIGN}{len(self.buffer.multi)}" if self.buffer.multi
            else {INSERT: "INS", VISUAL: "VIS", VBLOCK: "V-B",
    ...
    def _carets_markup(self, text: str, points: list) -> str:
        """A caret drawn at each of several insertion points."""
        out, cursor = [], 0
        caret = f"<span foreground='{self.CARET}'>|</span>"
        for point in points:
            point = max(0, min(point, len(text)))
            if point < cursor:
                continue
            out.append(esc(text[cursor:point]))
            out.append(caret)
            cursor = point
        out.append(esc(text[cursor:]))
        return "".join(out)

    def _block_markup(self, text: str, ranges: list) -> str:
        """Highlight each line's slice of the block, leaving the rest plain."""
        if not ranges:
            return esc(text)
        out, cursor = [], 0
        for start, end in ranges:
            ...
            # A zero-width slice on a short line still needs to show, or the
            # rectangle appears to have a hole in it.
            out.append(f"<span background='{self.SELECT}'>"
                       f"{esc(chunk) if chunk else ' '}</span>")
            cursor = end
        out.append(esc(text[cursor:]))
        return "".join(out)
    ...
    def _commit_rename(self, *_args) -> None:
        ...
        saved = titles.clean(self.rename_text)
        self._send({"op": ipc.OP_RENAME, "session_id": entry.session_id,
                    "title": self.rename_text})

        self.renaming = None
        self.rename_text = ""
        # Show the saved name on the row straight away, in the same trimmed
        # form the daemon will store. Waiting for the write and a re-read left
        # the row sitting in edit mode, and re-reading too early brought the
        # PREVIOUS name back for a moment.
        self._all_history = [replace(item, title=saved)
                             if item.session_id == entry.session_id else item
                             for item in self._all_history]
        self._refresh_search()              # back to a normal row, now
        self._toast("renamed" if saved else "name cleared")
    ...
    def _refresh_search(self) -> None:
        ...
        for index, entry in enumerate(hits):
            ...
            if self.renaming == index:
                # Drawn by us, not a Gtk.Entry. The window's key controller sits
                # in the CAPTURE phase and the entry never actually took focus,
                # so keystrokes went nowhere - the box looked editable and
                # accepted almost nothing. Every other text field here (the
                # search query, the `:` line) is already drawn this way.
                text = Gtk.Label(xalign=0, hexpand=True)
                text.set_use_markup(True)
                shown = esc(self.rename_text) or (
                    "<span alpha='45%'>name this chat</span>"
                    if not self.rename_text else "")
                text.set_markup(
                    f"{shown}<span foreground='{self.CARET}'>|</span>")
    ...
    def _on_key(self, _controller, keyval, _keycode, modifiers) -> bool:
        ...
        if ctrl:
            ...
            if key in COMMANDS:
                ...
                return True
            if key in ("ctrl-n", "ctrl-up", "ctrl-down") \
                    and not self.search_active and self.cmdline is None \
                    and self.focus_region == "input":
                self.buffer.feed(key)
                self._after_key()
                return True
            if key == "ctrl-a" and not self.search_active and self.cmdline is None:
            ...
            if key == "ctrl-v":
                ...
                return True
            if key == "ctrl-w":
                # Closes the tab in every mode. vim's insert-mode ctrl-w
                # (delete the previous word) moves to ctrl-backspace, which is
                # what every other text box on the desktop uses anyway.
    ...
    def _on_focus_change(self, *_args) -> None:
        # Nothing to do any more: the widget no longer takes hyprland's
        # keymap away while it is focused. Kept as the hook the window still
        # connects, so focus handling has somewhere to live if it needs it.
        return None
