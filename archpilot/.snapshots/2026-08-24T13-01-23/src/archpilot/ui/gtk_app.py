...
class ArchPilotWindow(Gtk.ApplicationWindow):
    ...
    def _render_attachments(self, paths: list) -> None:
        ...
        for index, path in enumerate(paths):
            ...
            if path.lower().endswith((".png", ".jpg", ".jpeg", ".webp", ".gif")):
                try:
                    # Gtk.Image with a pixel size, not Gtk.Picture: a Picture
                    # reports the image's own dimensions as its natural size
                    # and set_size_request only sets a MINIMUM, so every
                    # thumbnail came out a different size. pixel_size is a
                    # genuine fixed box.
                    child = Gtk.Image.new_from_file(path)
                    child.set_pixel_size(self.THUMB)
                    child.add_css_class("attachment-thumb")
    ...
    def _render_tabs(self, state: dict) -> None:
        ...
        for index, tab in enumerate(tabs):
            ...
            holder = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=1)
    ...
    def _message(self, turn: dict, index: int = 0) -> Gtk.Widget:
        ...
        if turn.get("streaming") and not (turn.get("answer") or "").strip():
            ...
            # one. Three pulsing dots say "it is coming" the way every chat
            # interface does.
            bubble.append(self._typing_dots())
        else:
    ...
    def _typing_dots(self) -> Gtk.Widget:
    ...
    def _fit_height(self) -> bool:
        ...
        wanted = max(self.MIN_HEIGHT, min(self.MAX_HEIGHT, natural.height + 8))
    ...
    def _on_key(self, _controller, keyval, _keycode, modifiers) -> bool:
        ...
        # With the help sheet pinned open, escape is what puts it away.
        if self.help_open and name == "escape":
            self.help_open = False
            self._render_cmdline()
            return True
