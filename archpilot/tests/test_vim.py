...
# --- typing on several lines happens LIVE, not on escape ---------------------

def _open_block_insert(text="one\ntwo\nthree", keys=("ctrl-v", "j", "j", "I")):
    buf = VimBuffer(mode=NORMAL)
    buf.text = text
    buf.cursor = 0
    for key in keys:
...
def test_each_keystroke_lands_on_every_line_immediately():
    ...
    buf = _open_block_insert()
    buf.feed("X")
    assert buf.text == "Xone\nXtwo\nXthree", "must not wait for escape"
    buf.feed("Y")
    assert buf.text == "XYone\nXYtwo\nXYthree"


def test_escape_does_not_apply_the_text_a_second_time():
    buf = _open_block_insert()
    buf.feed("X")
    buf.feed("escape")
    assert buf.text == "Xone\nXtwo\nXthree"
...
def test_backspace_erases_on_every_line_too():
    buf = _open_block_insert()
    for char in "XY":
        buf.feed(char)
    buf.feed("backspace")
...
def test_backspace_refuses_rather_than_deleting_unevenly():
    """At the very start of the lines there is nothing to erase. Deleting on
    some lines and not others is worse than doing nothing."""
    buf = _open_block_insert()
    buf.feed("backspace")
    assert buf.text == "one\ntwo\nthree"


def test_the_caret_sits_on_the_first_line_while_typing():
    buf = _open_block_insert()
    buf.feed("X")
    assert buf.cursor == 1


def test_every_edit_point_is_tracked_while_typing():
    buf = _open_block_insert()
    buf.feed("X")
    points, _count, _kind = buf.block_edit
    # "Xone\n" is 0..4, so line two starts at 5 and line three at 10; each
    # caret sits one character in.
    assert points == [1, 6, 11]


def test_lowercase_i_is_live_as_well():
    buf = _open_block_insert(keys=("ctrl-v", "j", "j", "i"))
    buf.feed("Z")
    assert buf.text == "Zone\nZtwo\nZthree"


def test_append_is_live():
    buf = _open_block_insert(keys=("ctrl-v", "j", "j", "A"))
    buf.feed("Z")
    assert buf.text == "oZne\ntZwo\ntZhree"


def test_change_is_live():
    buf = _open_block_insert(keys=("ctrl-v", "j", "j", "c"))
    buf.feed("Z")
    assert buf.text == "Zne\nZwo\nZhree"


def test_multicursor_typing_is_live_too():
    buf = VimBuffer(mode=NORMAL)
    buf.text = "foo bar foo"
    buf.cursor = 0
    for key in ("ctrl-n", "ctrl-n", "c"):
        buf.feed(key)
    buf.feed("Q")
    assert buf.text == "Q bar Q"


def test_undo_takes_back_the_whole_multi_line_insert():
    buf = _open_block_insert()
    for char in "XY":
        buf.feed(char)
    buf.feed("escape")
...
# --- escape leaves insert but keeps the cursors ------------------------------

def test_the_first_escape_keeps_the_cursors():
    """Two-step, as vim-visual-multi is: escape gets you out of typing without
    throwing away the cursors, so you can move and type again on every line."""
    buf = _open_block_insert()
    buf.feed("X")
    buf.feed("escape")
    assert buf.mode == NORMAL
    assert len(buf.multi) == 3


def test_normal_mode_commands_still_reach_every_line_after_that_escape():
    """`i` resumes exactly where the typing stopped; `a` goes one past, which
    is what those keys mean everywhere else in vim."""
    buf = _open_block_insert()
    buf.feed("X")
    buf.feed("escape")
    buf.feed("i")
    buf.feed("!")
    assert buf.text == "X!one\nX!two\nX!three"

    other = _open_block_insert()
    other.feed("X")
    other.feed("escape")
    other.feed("a")
    other.feed("!")
    assert other.text == "Xo!ne\nXt!wo\nXt!hree"


def test_the_second_escape_drops_the_cursors():
    buf = _open_block_insert()
    buf.feed("X")
    buf.feed("escape")
    buf.feed("escape")
...
def test_a_single_line_insert_keeps_no_cursors():
    """Only a genuinely multi-line edit leaves anything behind."""
    buf = VimBuffer(mode=NORMAL)
    buf.text = "one"
    buf.cursor = 0
    buf.feed("i")
...
def test_you_can_type_move_and_type_again_on_every_line():
    buf = _open_block_insert()
    buf.feed("X")
    buf.feed("escape")      # out of insert, cursors kept
    buf.feed("$")           # move each to its own line end
    buf.feed("a")
    buf.feed("!")
    assert buf.text == "Xone!\nXtwo!\nXthree!"
