...
func registerAll(r *action.Registry, a *App) {
	...
	// --- panes --------------------------------------------------------------
	reg("pane.toggle", "switch pane", func(action.Context) error { a.togglePane(); return nil })
	reg("pane.toggle_back", "switch pane backwards", func(action.Context) error {
		a.togglePane()
		return nil
	})
...
func (a *App) send() error {
	...
	if a.deps.Daemon != nil && !a.deps.Daemon.SendReady() {
		return fmt.Errorf("the sync process is not accepting sends yet")
	}

	text := a.composer.Take()
	local := a.pane.AddOptimistic(text)
	...
	a.pending = func() tea.Msg {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		defer cancel()

		sent, err := client.SendText(ctx, wacli.SendTextRequest{To: chat.JID, Message: text})
		if err != nil {
			return sendFailMsg{localID: local.ID, text: text, err: err}
		}
		return sentMsg{localID: local.ID, realID: sent.ID}
	}
	return nil
}
...
func (a *App) toggleChat(what string) error {
	...
	a.pushUndo(label, inverse)
	a.setStatus(label)
	_ = a.list.Invalidate(ctx, jid)
	return nil
}
